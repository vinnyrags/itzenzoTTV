...
function adaptSession(wp) {
    ...
    return {
        ...
        duck_race_channel_message_id: wp.duckRaceChannelMessageId ?? null,
...
/**
 * Race startability check. Originally this atomically transitioned the
 * session from 'open' → 'racing' to prevent double-starts, but the
 * race is now operator-controlled (admin-only `/duckrace start` slash
 * command) and the operator may legitimately want to:
 *
 *   - Run a race mid-stream (queue still open)
 *   - Re-run a race after a previous winner was declared
 *   - Run multiple ad-hoc races on the same queue session
 *
 * Each race overwrites duck_race_winner_user_id with the latest
 * winner. Session status is no longer touched by the race lifecycle —
 * it remains tied to the queue's own open/closed cycle. The function
 * is kept as a thin compatibility shim so existing callers don't
 * change; it just confirms the session exists.
 */
export async function claimForRace(id) {
    try {
        ...
        if (e.status === 404) return { changes: 0 };
        throw e;
    }
}
...
/**
 * Persist the Discord message ID for the #duck-race channel's roster
 * embed. updateDuckRaceEmbed() in lib/duck-race-embed.js calls this
 * after posting a fresh embed so subsequent updates can edit-in-place.
 */
export async function setDuckRaceChannelMessage(messageId, queueId) {
    await botFetch(`/queue/sessions/${encodeURIComponent(queueId)}`, {
        method: 'PATCH',
        body: JSON.stringify({ duck_race_channel_message_id: String(messageId) }),
    });
    return { changes: 1 };
}
 ...
 *
 * `sessionId` moves the entry to a different queue session. Used by
 * /offline to carry unfinished entries forward into the next stream's
 * queue instead of orphaning them in the closed session.
 */
export async function updateEntry(entryId, { status, discordHandle, displayName, detailLabel, detailData, sessionId } = {}) {
    const payload = {};
