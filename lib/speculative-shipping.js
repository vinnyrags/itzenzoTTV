...
export async function settleSpeculativeBuyers() {
    ...
    // Surface a single homepage Activity Feed event when the scan
    // actually moved something. Skip when the scan was a no-op so the
    // feed isn't pulsing every /offline run with "0 invoices sent."
    // Counts only — no emails, no Discord ids, no PII.
    const totalActioned = dmed.length + unlinked.length;
    if (totalActioned > 0) {
        try {
            broadcast('activity.shipping.settled', {
                kind: 'shipping.settled',
                title: 'Shipping invoices sent',
                description: buildShippingSummary(dmed.length, unlinked.length),
                color: 'emerald',
                icon: '📬',
            ...
            // Broadcast failures must not break the settlement scan.
            console.warn('settleSpeculativeBuyers broadcast failed:', e.message);
        }
    }
...
/**
 * Build the Activity Feed copy for a settlement run. Pure, exported
 * so the test can pin the wording without the full async dance.
 */
export function buildShippingSummary(dmedCount, unlinkedCount) {
    const parts = [];
    if (dmedCount > 0) {
