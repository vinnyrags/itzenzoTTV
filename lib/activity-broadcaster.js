...
export function broadcastCouponDrop(description) {
    // Intentionally no `code` parameter or payload field — coupons are
    // word-of-mouth now, the Activity Feed signal that a coupon is active
    // is enough without revealing the redeemable code itself.
    safeBroadcast('activity.coupon.drop', envelope(
        'coupon.drop',
        'Coupon drop',
        description || 'A coupon is active',
        COLORS.coupon,
        '🎟️',
        {},
    ));
