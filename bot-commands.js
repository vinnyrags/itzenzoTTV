...
const messages = [
    ...
    // Message 5: Card Shop
    {
        ...
        description: [
            ...
            '',
            '**`/pull reset`** — Chase prize hit — close current box and open a fresh one with the configured slot count (`pb_total_slots` in shop settings, default 50). The slot picker on itzenzo.tv immediately shows 0/N for the new box.',
            '',
            '**`/pull replenish args:<count>`** — Add N slots to the active pull box without resetting (rare — for "running low, no chase yet" cases).',
            '> Example: `/pull replenish args:25`',
            '',
            '*The pull box is perpetual — it auto-creates from settings the first time someone hits the homepage Buy In modal. No `/pull open` ceremony.*',
            '',
